...
| `hdris` | `{ [key]: string }` | Name → HDRI path map, the vocabulary each series' own `hdri` field (below) and the top-level `swatch-hdri` field pick from. `js/three-viewer.js` never reads this directly — a caller (`js/series.js`'s `hdriPath()`, or `scripts/auto-render.js`'s own resolution) looks up the key and passes the resolved path as `options.hdri`; an omitted/unresolved key falls back to `three-viewer.js`'s own `DEFAULT_HDRI_SRC`. |
| `swatch-primitive` | `string` | Which built-in shape (`"cylinder"`/`"box"`/`"sphere"`/`"torus"`) the metal-sample swatch bars (`assets/metal-sample_<metal>.webp`) render — not any actual product. Defaults to `"cylinder"` if omitted. Tuned via `scene-tool.html`'s "Camera" save (it's saved together with `swatch-camera`, below, as one "swatch scene"). |
| `swatch-hdri` | `string` | Which `hdris` key the swatch renderer lights with — independent of any series' own `hdri`, since swatches aren't series-scoped. Tuned via `scene-tool.html`'s "HDRI" save. |
| `swatch-camera` | `{ rotation: number, tilt: number, zoom: number }` | The swatch renderer's camera orbit — same shape and semantics as a product's own `3d-viewer-camera-default` below. Replaced an earlier hardcoded per-shape "camera distance exceeds the frame" formula once the swatch renderer started sharing `buildThreeViewer`'s real orbit system (see `client-scripts.md`) instead of a bespoke one-off. |
...
| `hdri` | `string` | Which top-level `hdris` key this series renders under — resolved to a path via `js/series.js`'s `hdriPath()` and passed as `options.hdri` to every `window.EmjiveModelViewer(...)` call `js/main.js`/`js/product.js` make for this series' products. `scripts/auto-render.js` mirrors the same resolution for its icon/fallback-img/top-shot renders. `scripts/auto-render.js`'s `warnHdriDrift()` warns if a series declares a key the top-level `hdris` map doesn't have. |
...
Currently one series: **Bones** (`data/series/bones/products.json`), 8 entries (Furcula, Foramen, Disc, Marrow, suture, cartilage, rib-cage, rib-cage-extended), all category `"ring"`.
...
- **`3d-viewer-camera-default`** (`{ rotation: number, tilt: number, zoom: number }`) — default/reset camera framing, parsed by `js/three-viewer.js`'s `parseOrbitConfig()` (see `client-scripts.md`). `rotation` spins left/right, `tilt` is up/down (90 = eye-level), `zoom` is radius as a % of the distance that exactly frames the model's bounding sphere. Also inherited (`zoom` only, `rotation` forced to 0) by auto-render's top-shot renderer. Set by hand, or via `scene-tool.html` (`npm run scene-tool`) — drag the model (or type exact rotation/tilt/zoom values), tick "Camera," and save writes straight into this field. Same tool also handles the swatch renderer's analogous `swatch-camera` (a top-level field, above — swatches aren't tied to a product).
...
- **`assets.top-shot`** (`{ [metal]: string }`) — one 1024×1024 WebP per metal, straight-down camera, lit with the same studio HDRI every other render uses, with an invisible "finger" occluder plane baked into the render so the ring's own hidden-from-above geometry (the far side of a gap in the shank, the inner wall of the hole) doesn't show through once composited over a hand (see `tooling.md`'s `scripts/auto-render.js` section). Generated by `scripts/auto-render.js`, populated for all 8 current products. Read by the active series' hero bundle (`series/<slug>/hero.js`'s `updateHeroRings()`) — see `hero-hand-visibility` above.
- **`assets.photos`** (`{ src: string, description: string }[]`) — extra photography slides appended to the carousel after the model/icon slide. Empty array hides the carousel's nav entirely if it leaves only one slide. `description` renders under its photo on the product page as `"On display: " + description` (em-dash if empty, same convention as `weight`/`composition`) — see `client-scripts.md`/`styling.md`. Editable per-photo via `npm run json-tool`, which also owns `src` (upload/replace/remove) same as before.

## Current data gaps (so placeholder content doesn't get mistaken for a bug)

Verified directly against the file: Foramen, Disc, and Marrow have no `assets.photos`, empty `description`, and `price: 0` for every metal. Marrow also has `hero-hand-visibility.visible: false` (not yet positioned on the hero hand). Every product's `weight`/`composition` is empty for every metal, including Furcula's. Furcula itself has real prices for all three metals (220/350/240) and a real description/photos — it's the one fleshed-out example the other three haven't caught up to yet. All four now have `assets.xray` set (Foramen/Disc/Marrow picked theirs up after Furcula's was originally the only one).
