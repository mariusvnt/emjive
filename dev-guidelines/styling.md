...
One file, `css/style.css` (~2274 lines), plain CSS — no preprocessor, no CSS-in-JS. Line numbers below verified directly against the file; re-check them if the file has grown/shrunk a lot since.
...
`css/style.css` is 2274 lines and holds only what's shared across pages. **The homepage hero's styling is no longer here** — it moved wholesale into the featured series' own bundle at `series/<slug>/hero.css`, along with the `--reveal-pin-height` and `--reveal-xray-opacity` tokens it owns (see `pages.md`).
...
| 738–1125 | Selection / order page (grown a lot since the order-page rebuild and the per-row undo redesign — sub-map below) |
| 1126–1219 | Checkout bar — `launch-order.html`'s own "liquid glass" bar (`.order-checkout-bar`), not `.selection-bar` (see `pages.md`/`client-scripts.md`) |
| 1220–1961 | Product detail page (by far the largest — sub-map below) |
| 1962–2274 | Floating selection bar |
...
- 749 — empty state (`.selection-empty`, centered "No item selected." text + a plain-text `.selection-empty__back` link — no more bordered `.btn`)
...
- 803/830 — item rows (`.order-items`/`.order-item`) — a full-bleed dark panel (same `100vw` + `calc(50% - 50vw)` breakout `.product-detail__select` uses below), padded top *and* bottom so the panel bookends itself. Each row reuses the same inverting-band technique as `.selection-bar__row::before` (see "Recurring non-obvious techniques" below), suppressed via `opacity: 0` for a row's own `.is-pending-undo` state (below) rather than crossing it
- 1002 — the undo prompt (`.order-item__undo-prompt`) — a *third* crossfade state a row can swap into (same `inset: 0` recipe `.order-item__overlay`, at 952, already uses for Modify/Unselect), replacing content/overlay in place rather than collapsing the row and showing a shared bar elsewhere the way an earlier design did. `.is-pending-undo` wins over `.is-open` on the (otherwise tied) specificity purely by declaration order — driven by `js/selection-page.js`, see `client-scripts.md`
- 1056 — shipping pick (`.order-shipping__option`) — the size modal's own dot-indicator recipe, reused
- 1093 — terms acceptance (`.order-terms`) — text hugging the left edge, a square opacity-toggled tick-box (no border) hugging the right

**Product detail sub-map** (matches the numbered comments in both this file and `product.html`):

- 1288 — label bar
- 1383 — carousel (no rim click zones anymore — dropped in favor of drag-to-scroll plus tap-a-slide-to-center-it, all in JS; see `client-scripts.md`)
- 1490 — metal selection
- 1594 — select button
- 1619 — prose sections (description / characteristics / shipping)
- 1672 — size-selection modal (1766 standard sizes, 1843 custom size, 1903 size guide, 1936 confirm button). `launch-order.html`'s own Modify popup (`#modifyModal`) reuses this exact `.size-modal`/`.product-metals` markup and CSS rather than a page-specific copy — a couple of rules here (`#modifyModalTitleName`/`#modifyModalTitleType`, `.size-modal__content .product-metals`) are scoped to that one instance so product.html's own "Choose a size" modal is untouched

**Floating selection bar sub-map**: 2044 — summary row (always visible in the DOM/markup sense — see the scroll-driven entrance note below for the one page where it isn't always *shown*), 2181 — drawer (item list, height driven by an inline px value `js/selection-bar.js` sets — see `client-scripts.md`).
