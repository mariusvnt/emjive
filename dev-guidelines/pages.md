...
1. **Filter row** — dual-purpose, **rendered by `js/main.js`'s `renderHeaderFilter()`** from the active series' own category list, never hand-written — don't add markup for it, and don't assume there are three category tags, since each series declares its own subset (see `data.md`).
   - On `index.html` (`#productGrid` exists): a DINish "Filter gallery by" label plus one Geist Mono `.is-cat` tag per category, when the active series declares more than one. Exactly one category drops the label and renders that lone tag by itself, `.is-active` by default and inert (no `href`, nothing to actually filter). Zero categories hides the row entirely.
   - On every other page: a single DINish "Gallery" link (`class="is-info"`) back to `index.html#products`, unconditionally — regardless of category count, so there's always a way back to the grid from the nav.

   `css/style.css` needs an explicit `.site-header__filter[hidden] { display: none }` for the zero-category case to actually disappear, same trap as `.product-card[hidden]` below: the row's own `display: flex` is an author rule and beats the UA `[hidden]` regardless of specificity.
2. **Archives** (`archives.html`) — DINish, `class="is-info"`.
3. **Creation process** (`creation-process.html`) — same.

Selection inside this menu — which category tag(s) are active, or the lone leftover one's default state — is signaled by a small dot before the text, the same always-rendered/background-toggled recipe used for size/metal selection elsewhere on the site (see `styling.md`'s "Recurring non-obvious techniques"), not a color change. "Gallery", Archives and Creation process never carry a dot — only category tags do.
...
**Filtering hides already-built cards** (`card.hidden`) rather than re-rendering the grid. Re-rendering would destroy and recreate every three.js `WebGLRenderer` on each toggle. `.product-card[hidden] { display: none }` in `css/style.css` is required for the hiding to work at all, since the card's own `display: grid` beats the UA `[hidden]` rule — same trap `.product-card__figure > img[hidden]` exists for below, just on a different element.

**A card's own 3D viewer is now built/disposed lazily as it scrolls in and out of range** (`js/main.js`'s `buildViewerFor`/`disposeViewerFor`, two `IntersectionObserver`s — see `client-scripts.md`), not once for the whole grid at page load. Past the browser's per-page WebGL context budget `buildThreeViewer()` returns `null` and that card just keeps showing its icon — retried the next time it scrolls back into the build observer's range, not permanent. Added after eagerly building every product's viewer at once — a `WebGLRenderer`, a full glTF fetch/decode, an HDRI PMREM build, each — stopped being safe once the catalog grew past a handful of products (eight bones products, some tens of MB each, over 100MB combined): real devices crashed outright rather than just running slowly. `.product-card__figure > img[hidden] { display: none }` is the matching fix for the icon a lazily-built viewer replaces — without it the "hidden" icon stayed laid out and pointer-interactive on top of the canvas, silently swallowing every drag meant for the model.
...
No title, no footer — straight from the header into the full selection, in `js/selection-page.js`. Toggles between `#selectionEmpty` (now "No selected item" plus a plain-text "Back to gallery" link, both centered) and `#selectionList`, in order:

1. **Item rows** (`#selectionItems`, a `<ul class="order-items">`) — a full-bleed dark panel breaking out to the true viewport edge (same `100vw`/`calc(50% - 50vw)` technique `.product-detail__select` uses), padded top and bottom by the same amount so the panel bookends itself. Each row shows thumb + name.category.metal.size (one continuous label, ellipsis-free — it fade-masks instead when it overflows) + price at rest; hovering (mouse), tapping, or focusing swaps the whole row to a Modify/Unselect overlay (a click anywhere else on that same row while it's open reverts it). Unselect crossfades the row *in place* into its own "Undo" prompt — no collapse, no reflow, same `inset: 0` recipe the Modify/Unselect overlay itself uses — rather than a single shared bar, so several rows can be independently mid-undo at once, each labeled with its own item. Only if that row's own window runs out unused does it actually collapse (reflowing the rows below it, same principle as the floating selection bar's icon-collapse) — or, if it was the last row standing, the whole list crossfades straight into the empty state instead of collapsing first. Modify opens a second modal (`#modifyModal`) built from the **same** `.size-modal`/`.product-metals` classes and wiring `product.html`'s own Select button uses — a metal picker plus the standard/custom size picker, pre-filled to the item's current choice, saving via `window.EmjiveSelection.updateItem()` instead of `addItem()`.
