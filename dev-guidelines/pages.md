...
The `<nav>` block is now **byte-identical across all seven pages** (verified by hashing it in each). The one remaining difference anywhere in the header is `.site-header__brand`'s href: `#top` on `index.html`, `index.html#top` everywhere else. That has to stay — `index.html#top` on the index itself triggers a reload instead of a same-page jump. It's also what `js/main.js`'s brand-logo click handler keys off of: on `index.html`, `window.EmjiveFocus.goHome()` (`js/product-focus.js`, see `client-scripts.md`) intercepts the click, cancelling any in-flight grid snap and animating to the top instead of leaving it to the native anchor — the bare `#top` fragment is what makes that interception meaningful. Everywhere else `window.EmjiveFocus` doesn't exist and the link is a real cross-page navigation, so CSS `scroll-behavior: smooth` is only ever the mechanism on a fresh `index.html` load or on a page with no grid to snap in the first place.
