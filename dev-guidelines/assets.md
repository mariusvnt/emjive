...
- `hdri/` — environment HDRIs. Two today: `studio_kontrast_04_1k.hdr` (the `_2k.hdr` variant that used to sit alongside it was removed — the 1k version is the only one anything ever loaded) and `autumn_field_puresky_1k.hdr` (a 1024×512 downscale from an 8k EXR source). Resolved by name via `data/series.json`'s top-level `"hdris"` map (`studio`/`blue-pure-sky`), not a single hardcoded path — each series picks one via its own `"hdri"` key (`js/series.js`'s `hdriPath()`), and the swatch renderer's own choice is the separate top-level `"swatch-hdri"` key (see `data.md`). `js/three-viewer.js`'s `DEFAULT_HDRI_SRC` (still `studio_kontrast_04_1k.hdr`) is only the fallback of last resort for a caller that omits `options.hdri` entirely — `scripts/auto-render.js`'s bare harness is one such caller, since it never loads `js/series.js`. Loaded by `loadEnvironment(renderer, hdriSrc)`, used by the live site and by every one of `scripts/auto-render.js`'s renders — icons, swatches, and top shots alike (top shots used to be lit with a plain `RoomEnvironment` instead; that override was removed, see `procedures.md`).
