...
- `hdri/studio_kontrast_04_1k.hdr` and `_2k.hdr` — the studio environment HDRI. The 1k version is what's actually used, by `js/three-viewer.js`'s shared environment (the live site and `scripts/auto-render.js`'s icon/swatch renders both go through it); not used for top shots at all, which use a plain `RoomEnvironment` instead (see `procedures.md`).
