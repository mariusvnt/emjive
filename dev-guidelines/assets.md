...
- `lab-lens.webp` (1920×1080) — the hand-authored render behind the homepage's fixed magnifying-lens artefact (`css/style.css`'s Lab-lens section, `js/lens-artefact.js`). Already has a real alpha channel (fully transparent background, a semi-transparent glass disc, an opaque chrome arm) — that's load-bearing, not incidental: the disc's center/radius (`--lens-disc-d`/`-r` in `css/style.css`) were measured directly off this file's own alpha data, and a re-render would need re-measuring rather than just dropping a new file in at the same name/dimensions. Hand-added like `logo.svg`/`hand-pointer-thumb-opened.svg` above — no render pipeline involvement.
