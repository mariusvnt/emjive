...
assets/
  ...
  fonts/                               │
  hdri/                                │
  logo.svg, logo_compact.svg           │
  metal-sample_{steel,silver,bronze}.webp  ├─ global — every series shares these
...
- `lab-lens.webp` (1920×1080) — the hand-authored render behind the homepage's fixed magnifying-lens artefact (`css/style.css`'s Lab-lens section, `js/lens-artefact.js`), now specifically **frame 60** of the 60-frame extend sequence below — the settled, fully-extended state, referenced at this same path/filename so `index.html`'s `<img id="lensArtefact">` and every other existing consumer needed no changes when the sequence was added (or when it was re-rendered once already). Already has a real alpha channel (fully transparent background, a semi-transparent glass disc, an opaque chrome arm) — that's load-bearing, not incidental: the disc's center/radius (`--lens-disc-d`/`-r` in `css/style.css`) were measured directly off this file's own alpha data (center ≈ 960,544.5 of the 1920×1080 frame, radius 196px of the 1080px height — a re-render would need re-measuring the same way, not just dropping a new file in at the same name/dimensions; this particular re-render happened to measure out identical to the one before it, so no CSS change was needed that time). Hand-authored (Blender/Cycles) and, as of the most recent re-render, exported directly to WebP by the render tool itself — no PNG-to-WebP conversion step in this repo any more.
- `lab-lens-frames/` — the other 59 frames of that same sequence (`lab-lens_01.webp`…`_59.webp`, 2-digit zero-padded), played back by `js/lens-artefact.js` to animate the glass extending into place on arrival at the grid and retracting on the way back into the hero — see that file's own doc in `client-scripts.md`. No numbered-frame-sequence convention existed elsewhere in this repo before this; global rather than series-scoped for the same reason `lab-lens.webp` itself is (one lens, not per-series art).
