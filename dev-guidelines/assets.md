...
- `lab-lens.webp` (1920×1080) — the hand-authored render behind the homepage's fixed magnifying-lens artefact (`css/style.css`'s Lab-lens section, `js/lens-artefact.js`), now specifically **frame 60** of the 60-frame extend sequence below — the settled, fully-extended state, referenced at this same path/filename so `index.html`'s `<img id="lensArtefact">` and every other existing consumer needed no changes when the sequence was added. Already has a real alpha channel (fully transparent background, a semi-transparent glass disc, an opaque chrome arm) — that's load-bearing, not incidental: the disc's center/radius (`--lens-disc-d`/`-r` in `css/style.css`) were measured directly off this file's own alpha data (center ≈ 959,542.5 of the 1920×1080 frame, radius 196px of the 1080px height — a re-render would need re-measuring the same way, not just dropping a new file in at the same name/dimensions). Blender/Cycles-rendered PNG converted to WebP via `sharp({ quality: 92, alphaQuality: 100 })` — `scripts/auto-render.js`'s own encode settings, reused by hand for a one-off conversion script (not committed to `scripts/`, same "hand-added, no render pipeline involvement" precedent `logo.svg`/`hand-pointer-thumb-opened.svg` above set).
- `lab-lens-frames/` — the other 59 frames of that same sequence (`lab-lens_01.webp`…`_59.webp`, 2-digit zero-padded, same WebP settings as `lab-lens.webp` above), played back by `js/lens-artefact.js` to animate the glass extending into place on arrival at the grid and retracting on the way back into the hero — see that file's own doc in `client-scripts.md`. No numbered-frame-sequence convention existed elsewhere in this repo before this; global rather than series-scoped for the same reason `lab-lens.webp` itself is (one lens, not per-series art).
